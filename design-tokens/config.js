/**
 * Style Dictionary v4 configuration for Luminous design tokens.
 *
...
function cssLuminousVariables({ dictionary }) {
  return `/**
...
function swiftLuminousExtensions({ dictionary }) {
  ...
  const fontSizeTokens = dictionary.allTokens.filter(p =>
...
function javascriptLuminousTailwind({ dictionary }) {
  const colors = {};
  const spacing = {};
  const borderRadius = {};
...
module.exports = {
  ...
  hooks: {
    formats: {
      'css/luminous-variables': cssLuminousVariables,
      'swift/luminous-extensions': swiftLuminousExtensions,
      'compose/luminous-object': composeLuminousObject,
      'javascript/luminous-tailwind': javascriptLuminousTailwind,
    }
  },
  platforms: {
    css: {
